  ...
  shipTypes.forEach((shipType) => {
    ...
    while (!placed && attempts < maxAttempts) {
      ...
      if (canPlaceShip(board, row, col, shipType.length, orientation)) {
        ...
        placed = true;
        console.log(
          `Placed ${shipType.name} at (${row}, ${col}) ${orientation}`,
        );
      }
      attempts++;
    }
    ...
    if (!placed) {
      console.error(
        `Failed to place ${shipType.name} after ${maxAttempts} attempts`,
      );
    }
  });
}
...
function placeShip(board, row, col, shipType, orientation) {
  ...
  for (let i = 0; i < shipType.length; i++) {
    ...
    if (board === playerBoard && board.board[currentRow][currentCol].element) {
      board.board[currentRow][currentCol].element.style.backgroundColor =
        "#4dabf7";
  ...
  for (let row = 0; row < size; row++) {
    for (let col = 0; col < size; col++) {
      ...
      cell.addEventListener("click", () =>
        handleCellClick(row, col, cell, boardType),
      );
